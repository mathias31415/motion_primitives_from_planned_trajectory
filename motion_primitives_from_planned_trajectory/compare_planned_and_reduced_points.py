...
def plot_cartesian_trajectory(filepath_planned, filepath_reduced, pose_names):
    ...
    first_quat = [df_reduced.iloc[0][qx], df_reduced.iloc[0][qy], df_reduced.iloc[0][qz], df_reduced.iloc[0][qw]]
    first_rot = R.from_quat(first_quat)
    first_pt = [x.iloc[0], y.iloc[0], z.iloc[0]]

    ax.quiver(*first_pt, *first_rot.apply([1, 0, 0]), length=arrow_len, color='r', linestyle='dashed', normalize=True)
    ax.quiver(*first_pt, *first_rot.apply([0, 1, 0]), length=arrow_len, color='g', linestyle='dashed', normalize=True)
    ...
    plt.tight_layout()
     ...
     # Save figure
    base_name = os.path.basename(filepath_planned).replace('_planned.csv', '_compare_planned_vs_reduced_LIN_cartesian.png')
...
def plot_joint_trajectory(filepath_planned, filepath_reduced, joint_names):
    ...
    fig.legend(['Planned', 'Reduced'],
     ...
     # Save figure
    base_name = os.path.basename(filepath_planned).replace('_planned.csv', '_compare_planned_vs_reduced_PTP_joint.png')
    plot_path = os.path.join(os.path.dirname(filepath_planned), base_name)
    plt.savefig(plot_path)
    plt.show()
    print(f"Figure with planned and reduced points comparison saved to: {plot_path}")
